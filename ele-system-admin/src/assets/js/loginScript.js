...
console.log(
    ...
    "background:#f5f5f5; color:#333; padding: 1px; border-radius: 3px 0 0 3px;",
...
function pointerPrototype() {
    ...
    this.down = false;
    this.moved = false;
    // 正确：对象类型（0~1 范围，与 generateColor 统一）
    this.color = { r: 30/255, g: 30/255, b: 30/255 };
}

// ===== 自动随机滑动的虚拟指针配置（多指针版） =====
const AUTO_POINTER_COUNT = 4; // 自动指针数量，可自行调整（比如5个）
const AUTO_MOVE_SPEED = 0.004; // 每帧移动的步长（数值越小，滑动越平缓）
let autoPointers = []; // 自动指针数组
let autoTargets = [];  // 每个自动指针的目标位置数组
...
// 给指定自动指针设置新的随机目标位置（接收指针索引）
function setAutoTarget(pointerIndex) {
    autoTargets[pointerIndex] = {
        x: Math.random(), // 随机X（0~1）
        y: Math.random()  // 随机Y（0~1）
    };
    autoPointers[pointerIndex].color = generateColor(); // 换颜色
}

// 初始化多个自动指针（替代原 setAutoTarget() 单行初始化）
function initAutoPointers() {
    autoPointers = [];
    autoTargets = [];
    for (let i = 0; i < AUTO_POINTER_COUNT; i++) {
        // 创建单个自动指针
        const pointer = new pointerPrototype();
        pointer.down = true; // 持续按下
        pointer.texcoordX = Math.random(); // 初始位置随机
        pointer.texcoordY = Math.random();
        autoPointers.push(pointer);
        // 初始化目标位置
        setAutoTarget(i);
    }
}
...
function startGUI() {
    ...
    github.__li.style.borderLeft = '3px solid #333';
    ...
    twitter.__li.style.borderLeft = '3px solid #333';
    ...
    discord.__li.style.borderLeft = '3px solid #333';
...
var displayShaderSource = "\n    precision highp float;\n    precision highp sampler2D;\n\n    varying vec2 vUv;\n    varying vec2 vL;\n    varying vec2 vR;\n    varying vec2 vT;\n    varying vec2 vB;\n    uniform sampler2D uTexture;\n    uniform sampler2D uBloom;\n    uniform sampler2D uSunrays;\n    uniform sampler2D uDithering;\n    uniform vec2 ditherScale;\n    uniform vec2 texelSize;\n\n    vec3 linearToGamma (vec3 color) {\n        color = max(color, vec3(0));\n        return max(1.055 * pow(color, vec3(0.416666667)) - 0.055, vec3(0));\n    }\n\n    void main () {\n        vec3 c = texture2D(uTexture, vUv).rgb;\n\n    #ifdef SHADING\n        vec3 lc = texture2D(uTexture, vL).rgb;\n        vec3 rc = texture2D(uTexture, vR).rgb;\n        vec3 tc = texture2D(uTexture, vT).rgb;\n        vec3 bc = texture2D(uTexture, vB).rgb;\n\n        float dx = length(rc) - length(lc);\n        float dy = length(tc) - length(bc);\n\n        vec3 n = normalize(vec3(dx, dy, length(texelSize)));\n        vec3 l = vec3(0.0, 0.0, 1.0);\n\n        // 【修改】降低漫反射亮度（避免彩色泛白），从 0.7→1.0 改为 0.8→1.0\n        float diffuse = clamp(dot(n, l) + 0.8, 0.8, 1.0);\n        c *= diffuse;\n    #endif\n\n    #ifdef BLOOM\n        vec3 bloom = texture2D(uBloom, vUv).rgb;\n    #endif\n\n    #ifdef SUNRAYS\n        float sunrays = texture2D(uSunrays, vUv).r;\n        c *= sunrays;\n    #ifdef BLOOM\n        bloom *= sunrays;\n    #endif\n    #endif\n\n    #ifdef BLOOM\n        float noise = texture2D(uDithering, vUv * ditherScale).r;\n        noise = noise * 2.0 - 1.0;\n        bloom += noise / 255.0;\n        bloom = linearToGamma(bloom);\n        // 【修改】降低bloom强度（避免彩色泛白），从 c += bloom 改为 c += bloom * 0.8\n        c += bloom * 0.8;\n    #endif\n\n        float a = max(c.r, max(c.g, c.b));\n        gl_FragColor = vec4(c, a);\n    }\n";
...
function update() {
    ...
    if (!config.PAUSED) {
    // 遍历每个自动指针
    autoPointers.forEach((pointer, index) => {
        const target = autoTargets[index];
        // 计算当前指针到目标位置的距离
        const distX = target.x - pointer.texcoordX;
        const distY = target.y - pointer.texcoordY;
        const distance = Math.hypot(distX, distY);

        if (distance > AUTO_MOVE_SPEED) {
            // 向目标移动一步
            pointer.prevTexcoordX = pointer.texcoordX;
            pointer.prevTexcoordY = pointer.texcoordY;
            pointer.texcoordX += distX * (AUTO_MOVE_SPEED / distance);
            pointer.texcoordY += distY * (AUTO_MOVE_SPEED / distance);
            // 计算增量并触发流体
